    ...
    try {
      ...
      if (externalResponse.ok) {
        ...
        if (externalResults.length > 0) {
          ...
          // Fetch metadata for each result from the summarize endpoint
            ...
            externalResults.slice(0, k).map(async (result) => {
              try {
                ...
                if (summaryData.success) {
                  // The summary API might return structured data, let's parse it
                  const rawData = summaryData.rawData || {};
                  
                  return {
                    ...result,
                    // Use abstract from API if available, otherwise use summary as abstract
                    abstract: rawData.abstract || rawData.description || summaryData.summary || result.abstract,
                    // Extract authors from API response
                    authors: rawData.authors || rawData.creators || result.authors,
                    // Extract year from API response
                    year: rawData.year || rawData.publication_year || result.year,
                    // Store full summary separately
                    fullSummary: summaryData.summary,
                  };
                }
              } catch (error) {
                console.warn(`Failed to fetch metadata for ${result.filename}:`, error);
              }
              return result;
            })
          );
...
export const getExternalSummary = async (filename) => {
  try {
    ...
    return {
      filename: filename,
      summary: summaryText,
      rawData: typeof data === "object" ? data : {}, // Store raw data for metadata extraction
